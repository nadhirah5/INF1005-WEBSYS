...
<html lang="en">
    ...
    <body>
        ...
        function fetchMenuBanner() {
            global $foodID, $menuBannerDescription, $menuBannerPicture, $errorMsg, $success, $result;
...
// Prepare the statement:
                $stmt = $conn->prepare("SELECT * FROM menuBanner");
// Bind & execute the query statement:
        ...
        function fetchMenu() {
            global $foodID, $foodName, $foodPrice, $foodPicture, $errorMsg, $success, $result2;
// Create database connection.
            $config = parse_ini_file('../../private/db-config.ini');
            $conn = new mysqli($config['servername'], $config['username'],
                    $config['password'], $config['dbname']);
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        "<img class='' src='images/" . $row["menuBannerPicture"] . "' alt='" . $row["menuBannerDescription"] . "' style='height=500;width:100%;'>" .
        "<div class='text'>" . $row["menuBannerDescription"] . "</div>" .
        "</div>";
    }
    ...
    while ($currentrow <= $numrows) {
        echo "<span class='dot' onclick='currentSlide(" . $currentrow . ")'></span>";
        $currentrow += 1;
    }
    echo "</div>";
} else {
...
if ($result2->num_rows > 0) {
    ...
    "<div class=' d-flex pb-4 justify-content-left' >" .
    "<input type='file' name='foodPicture' required>" .
    "</div>" .
    "<label>Food Name</label><br>" .
    "<div class=' d-flex pb-4 justify-content-left' >" .
    "<input type='text' name='foodName' required>" .
    "</div>" .
    "<label>Food Pricing</label><br>" .
    "<div class=' d-flex pb-4 justify-content-left' >" .
