...
function sanitize_money($data) {
    ...
    $foodName = sanitize_input($_POST['drinkName']);
    $foodPrice = sanitize_money($_POST['drinkPrice']);
    $drinkQuantity = sanitize_menu($_POST['drinkQuantity']);
    $IceQuantity = sanitize_menu($_POST['IceQuantity']);
    ...
    if (!isset($_SESSION['cart_items'])) {
        ...
        foreach ($_SESSION['cart_items'] as $index => $cartItem) {
            if ($cartItem['foodName'] === $foodName && $cartItem['request'] === $request) {
                ...
                $_SESSION['cart_items'][$index]['foodQuantity'] += $drinkQuantity;     
                $itemExists = true;
                break;
            }
        }
